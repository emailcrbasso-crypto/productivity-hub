...
import { QUADRANTS, metaOf, quadrantOf, type ImpactEffortTask, type Quadrant } from "./types";
import { deleteTask, moveTask, toggleComplete } from "./actions";

type Props = {
  task: ImpactEffortTask;
  onEdit: () => void;
  ...
  function handleToggle() {
    startTransition(async () => {
      ...
      if (result.awarded && onAfterToggle) {
        ...
        if (result.unlockedTitles.length > 0) parts.push(`🏆 ${result.unlockedTitles.join(", ")}`);
  ...
  return (
    ...
    >
      ...
      <div className="relative" ref={menuRef}>
        ...
        {menuOpen ? (
          <div className="absolute right-0 z-10 mt-1 w-52 rounded-md border border-zinc-200 bg-white py-1 text-xs shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
            <button
              type="button"
              onMouseDown={(e) => {
                e.preventDefault();
